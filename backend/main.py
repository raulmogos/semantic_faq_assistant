...
async def list_sessions(request: Request) -> list[SessionSummary]:
    ...
    return await request.app.state.agent.list_sessions()
...
async def get_session(session_id: str, request: Request) -> list[ConversationMessage]:
    ...
    return await request.app.state.agent.get_session_history(session_id)


@app.delete("/sessions/{session_id}", status_code=204)
async def delete_session(session_id: str, request: Request) -> None:
    """Delete a session and all its associated data."""
    await request.app.state.agent.delete_session(session_id)
